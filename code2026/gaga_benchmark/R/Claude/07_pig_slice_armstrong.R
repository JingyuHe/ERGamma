#!/usr/bin/env Rscript
# 07_pig_slice_armstrong.R - PIG-augmented MCMC with slice sampling on log α.
# Replaces gaga's Stirling approximation with exact PIG MCMC; otherwise the
# model and hyperparameters are identical to gaga::fitGG.
...
source(file.path(src_dir, "pig_gaga_slice_lib.R"))
...
n_iter <- as.integer(Sys.getenv("PIG_SLICE_ITER", unset = "200"))
n_burn <- as.integer(Sys.getenv("PIG_SLICE_BURN", unset = "60"))
trunc  <- as.integer(Sys.getenv("PIG_SLICE_TRUNC", unset = "30"))
slice_w <- as.numeric(Sys.getenv("PIG_SLICE_W", unset = "1.0"))
chkpt_every <- as.integer(Sys.getenv("PIG_SLICE_CHKPT", unset = "10"))
seed <- as.integer(Sys.getenv("PIG_SLICE_SEED", unset = "42"))
...
gg_calls <- as.logical(gaga::findgenes(gg_fit, X_log, groups,
                                       fdrmax = 0.05, parametric = TRUE)$d != 0)
gg_score <- 1 - gg_fit$pp[, 1]
cat(sprintf("[ref] hyper a0=%.3f nu=%.3f b_alpha=%.3f nu_alpha=%.3g #DE=%d\n",
            gg_par$a0, gg_par$nu, gg_par$balpha, gg_par$nualpha, sum(gg_calls)))

# limma_BH
fit_l <- limma::eBayes(limma::lmFit(X_log, stats::model.matrix(~ groups)))
limma_calls <- p.adjust(fit_l$p.value[, 2], "BH") <= 0.05
limma_score <- -log10(pmax(fit_l$p.value[, 2], 1e-300))
...
set.seed(seed)
ckpt <- result_file(paste0("pig_slice_armstrong_",
                           data_choice, "_", transform_choice, "_ckpt.rds"))
cat(sprintf("\n[PIG-slice] iter=%d burn=%d trunc=%d w=%.2f ...\n",
            n_iter, n_burn, trunc, slice_w))
t0 <- Sys.time()
out <- pig_gaga_slice_mcmc(X = X, group = groups, patterns = patterns,
...
cat(sprintf("[PIG-slice] elapsed=%.1fs  median alpha=%.2f  #DE=%d\n",
            elapsed_pig, median(out$alpha_mean), sum(pig_calls)))
...
agreement <- rbind(
  agree(pig_calls, gg_calls, "PIG_slice", "gaga_fitGG"),
  agree(pig_calls, limma_calls, "PIG_slice", "limma_BH"),
  agree(gg_calls, limma_calls, "gaga_fitGG", "limma_BH"))

summary_df <- data.frame(
  method = c("PIG_slice", "gaga_fitGG", "limma_BH"),
  ...
  mean_abs_pp_diff_vs_gaga = c(
    mean(abs(out$pp[, 1] - gg_fit$pp[, 1])),
    0.0, NA),
  stringsAsFactors = FALSE)

suffix <- paste("slice", data_choice, transform_choice, sep = "_")
write.csv(summary_df,
  result_file(paste0("pig_slice_armstrong_summary_", suffix, ".csv")),
  row.names = FALSE)
write.csv(agreement,
  result_file(paste0("pig_slice_armstrong_agreement_", suffix, ".csv")),
  row.names = FALSE)
saveRDS(list(out = out, gg_calls = gg_calls, gg_score = gg_score,
        ...
        result_file(paste0("pig_slice_armstrong_", suffix, ".rds")))

cat("\n=== PIG-slice Armstrong ===\n")
print(summary_df, row.names = FALSE)
cat("\n=== Agreement ===\n")
print(agreement, row.names = FALSE)
cat(sprintf("\nMean |pp_slice - pp_gaga| = %.4f  (Stirling vs PIG bias on the same model)\n",
            mean(abs(out$pp[, 1] - gg_fit$pp[, 1]))))
