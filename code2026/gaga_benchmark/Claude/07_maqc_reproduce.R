#!/usr/bin/env Rscript
# =============================================================================
# 07_maqc_reproduce.R  (Claude/ debugged version)
#
# Reproduces Rossell (2009) Section 7 / Figure 2(b): MAQC titration analysis on
# the first-site Affymetrix hgu133plus2 arrays, validating against the qPCR
# (TaqMan, GPL4097) measurements. Same logic as gaga_benchmark/R/07; this copy
# uses the Claude/ benchmark_lib (no Mach-O path issues, fixed limma helper).
# =============================================================================
...
read_series_metadata <- function(file) {
  ...
  data.frame(
    gsm = out$geo_accession, title = out$title,
...
make_affy_symbol_map <- function(gpl570) {
  if (!"Gene symbol" %in% names(gpl570))
...
# Titration patterns (column order A, C, D, B by titration level):
#   row 0: all equal
#   row 1: A vs (C, D, B)
#   row 2: (A, C) vs (D, B)
#   row 3: (A, C, D) vs B
#   row 4: titration (all distinct)
...
fit_gaga_maqc <- function(x, groups, nclust, method = "EM") {
  ...
  for (fm in fit_methods) {
    fit <- tryCatch(
      ...
      error = function(e) {
        errors <<- c(errors, paste0("fitGG/", fm, ": ", conditionMessage(e)))
        NULL
      })
    if (is.null(fit)) next
    fit <- tryCatch(
      ...
      error = function(e) {
        errors <<- c(errors, paste0("parest/", fm, ": ", conditionMessage(e)))
        NULL
      })
...
# Ga: try positive-scale data first (correct), only fall back to log2 if the
# raw fit errors out.
fit_ga_maqc <- function(x, groups) {
  ...
  for (nm in names(attempts)) {
    xx <- attempts[[nm]]
    fit <- tryCatch(
      EBarrays::emfit(data = xx, family = "GG", hypotheses = patterns,
                      num.iter = 20, verbose = FALSE),
      error = function(e) {
        errors <<- c(errors, paste0(nm, ": ", conditionMessage(e))); NULL
      })
    if (!is.null(fit)) {
  ...
  stop("Ga fit failed: ", paste(errors, collapse = " | "))
}
...
collapse_probe_symbols <- function(affy_map) {
  stats::aggregate(symbol ~ probe_id, data = affy_map,
                   FUN = function(z) paste(unique(z), collapse = ";"))
}
top_pattern4_lists <- function(fits, affy_fit, groups, probe_symbols, n = 1000) {
  means <- sapply(c("A", "B"),
                  function(g) rowMeans(affy_fit[, groups == g, drop = FALSE]))
  rownames(means) <- rownames(affy_fit)
  rows <- list(); rid <- 1
  for (m in names(fits)) {
    pp4 <- fits[[m]]$pp[, 5]; names(pp4) <- rownames(affy_fit)
    for (dir in c("B_gt_A", "A_gt_B")) {
      keep <- if (dir == "B_gt_A") means[, "B"] > means[, "A"]
              else means[, "A"] > means[, "B"]
      ord <- order(pp4[keep], decreasing = TRUE)
      ids <- names(pp4[keep])[ord][seq_len(min(n, sum(keep)))]
      out <- data.frame(method = m, direction = dir, rank = seq_along(ids),
                        probe_id = ids, pattern4_prob = pp4[ids],
                        mean_A = means[ids, "A"], mean_B = means[ids, "B"],
                        log2_B_over_A = log2(
                          pmax(means[ids, "B"], .Machine$double.eps) /
                          pmax(means[ids, "A"], .Machine$double.eps)),
                        stringsAsFactors = FALSE)
      out <- merge(out, probe_symbols, by = "probe_id", all.x = TRUE,
                   sort = FALSE)
      out <- out[order(out$rank), ]
      rows[[rid]] <- out; rid <- rid + 1
    }
  }
  do.call(rbind, rows)
}
...
if (n_genes > 0 && n_genes < nrow(affy_1)) {
  ...
  vars <- if (requireNamespace("matrixStats", quietly = TRUE))
    matrixStats::rowVars(log_affy_1) else apply(log_affy_1, 1, stats::var)
...
message("Fitting GaGa on ", nrow(affy_fit), " probes and ",
        ncol(affy_fit), " arrays.")
gg_fit <- fit_gaga_maqc(affy_fit, groups_affy, nclust = 1, method = gaga_method)
message("Fitting MiGaGa2 on ", nrow(affy_fit), " probes and ",
        ncol(affy_fit), " arrays.")
mg_fit <- fit_gaga_maqc(affy_fit, groups_affy, nclust = 2, method = gaga_method)
message("Fitting Ga/Gamma-Gamma on ", nrow(affy_fit), " probes and ",
        ncol(affy_fit), " arrays.")
ga_error <- NA_character_
ga_fit <- tryCatch(fit_ga_maqc(affy_fit, groups_affy),
                   error = function(e) {
                     ga_error <<- conditionMessage(e)
                     warning("Skipping Ga/Gamma-Gamma: ", ga_error); NULL
                   })
...
pattern_counts$diff_from_paper <- pattern_counts$count -
                                  pattern_counts$paper_count
...
top_pattern4 <- top_pattern4_lists(
  ...
  affy_fit = affy_fit, groups = groups_affy,
  probe_symbols = probe_symbols, n = 1000
)

curve_rows <- list(); score_rows <- list(); summary_rows <- list()
for (method in names(method_scores)) {
  dat <- merge(qpcr_info, method_scores[[method]],
               by = "symbol", all.x = FALSE, sort = FALSE)
  curve <- validation_curve(dat$score, dat$validated)
  curve$method <- method; curve_rows[[method]] <- curve
  dat$method <- method; score_rows[[method]] <- dat
  final <- curve[nrow(curve), ]
  summary_rows[[method]] <- data.frame(
    method = method, n_qpcr_assays = nrow(qpcr_info),
...
data_summary <- data.frame(
  affy_file = affy_file, qpcr_file = qpcr_file,
  n_affy_probes = nrow(affy), n_affy_samples = ncol(affy),
  n_first_site_arrays = ncol(affy_1),
  first_site_titles = paste(affy_meta_1$title, collapse = ","),
  n_fit_probes = nrow(affy_fit),
  n_qpcr_assays = nrow(qpcr), n_qpcr_samples = ncol(qpcr),
  ...
  ga_method_used = if (is.null(ga_fit)) NA_character_
                   else ga_fit$method_used,
  ga_error = ga_error,
...
write.csv(data_summary, result_file("maqc_data_summary.csv"),
          row.names = FALSE)
...
write.csv(top_pattern4, result_file("maqc_top_pattern4_genes.csv"),
          row.names = FALSE)
...
print(data_summary); print(pattern_counts); print(roc_summary)
